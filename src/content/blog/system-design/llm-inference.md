...
请求 A 很快完成。请求 B 先要读完 20,000 个 prompt token，之后还要执行最多 1,000 轮 decode。如果调度器只限制“同时 32 个请求”，那么 32 个 B 就可能把 KV cache 塬满；如果把 A 排在 B 后面，A 的首字延迟会被一段超长 prefill 拖住。
